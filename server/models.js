...
const findTrails = (latitude, longitude, geom2, decimal1) => db.query(`SELECT trails.TrailCn AS id, trails.TrailName AS name, trails.GISMiles AS length, trails.GEOM AS coordinates FROM trails WHERE (acos(sin(RADIANS(${latitude})) * sin(RADIANS(CAST(SUBSTRING(GEOM, 33, 10) AS DECIMAL(11, 8)))) + cos(RADIANS(${latitude})) * cos(RADIANS(CAST(SUBSTRING(GEOM, 33, 10) AS DECIMAL(11, 8)))) * cos(RADIANS(CAST(SUBSTRING(GEOM, 13, ${geom2}) AS DECIMAL(${decimal1}, 8)) - (${longitude})))) * 6371 <= 70)`, {type: db.QueryTypes.SELECT});
...
module.exports.getRecAddressModel = (recAreaID) => schemas.recAreaAddress.findOne({
...
module.exports.getFacilityAddressModel = (facilityID) => schemas.facilitiesAddress.findOne({
...
module.exports.getRecActivitiesModel = (recAreaID) => schemas.recAreas.findOne({
    where: { RecAreaID: recAreaID },
    include: [{ model: schemas.activities }],
  });

 // Get Activities for a Facility - Model
module.exports.getFacilitiesActivitiesModel = (facilityID) => schemas.facilities.findOne({
    where: { FacilityID: facilityID },
    include: [{ model: schemas.activities }],
  });
...
module.exports.trailsAndActivitiesWithinRadiusOfFacilityModel = (latitude, longitude, facilityID) => {
  ...
  if (longitude <= -100) {
    ...
    .then((facilityActivities) => entityInfo(facilityActivities, listOfTrails));
  } else {
...
module.exports.trailsAndActivitiesWithinRadiusOfRecAreasModel = (latitude, longitude, recAreaID) => {
  ...
  if (longitude <= -100) {
    ...
    .then((recAreaActivities) => entityInfo(recAreaActivities, listOfTrails))
...
module.exports.getRecAreaModel = (recAreaID) => schemas.recAreas.findOne({
    where: {RecAreaID: recAreaID},
    include: [
...
module.exports.getFacilityModel = (facilityID) => schemas.facilities.findOne({
    where: {FacilityID: facilityID},
    include: [
...
module.exports.getActivitiesModel = (activity) => schemas.activities.findOne({
