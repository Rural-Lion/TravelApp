...
getRecActivitiesModel = require('../../models.js').getRecActivitiesModel;

describe('getRecActivitiesModel', () => {
  const result = getRecActivitiesModel(6);

  it('Should be a function', () => {
    should.exist(getRecActivitiesModel);
    getRecActivitiesModel.should.be.a('function');
  });
  ...
  it('Should have the correct facility\'s activity list', (done) => {
    result.then((data) => {
      data.activities.length.should.equal(13);
      data.activities[0].entityactivities.EntityID.should.equal(6);
      done();
    });
  });
});
